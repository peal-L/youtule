...
Page({
    ...
    saveToAlbum() {
        ...
        that.loading.show();
        setTimeout(function() {
            let img = that.data.img.url;
            // 计算canvas高度
            let width = that.data.img.width;
            let height = that.data.img.height;
            that.setData({
                canvas: {
                    width: width,
                    height: height
            ...
            ctx.draw(false, function() {
                ...
                wx.canvasGetImageData({
                    ...
                    success(res) {
                        ...
                        let data = that.convertToBlur(res);
                ...
                function save() {
                    wx.saveImageToPhotosAlbum({
                        ...
                        success() {
                            ...
                            // 清空画布 释放缓存
                            ctx.clearRect(0, 0, that.data.canvas.width, that.data.canvas.height);
                            ctx.draw();
                        },
                        fail() {
                            that.tip.show({
